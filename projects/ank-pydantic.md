...
The Network Modeling & Configuration Library represents network topologies as typed Python objects backed by a Rust graph engine. You define nodes, edges, and layers using Pydantic models. The library stores them in a Rust-native topology engine (`[ank_nte](../ank_nte)`) and exposes a composable query API that builds lazy evaluation plans in Python and executes them in Rust.
